...
public class CsvUtil {
    ...
    public CsvUtil(final ResourceLoader resourceLoader, @Value("classpath:/static/${websites.company.names}") final String websitesCompanyNamesFilename, @Value("classpath:/static/${websites.domain}") final String websitesDomainFilename) {
    ...
    public List<String> readDomainsFromInputStream() {
        ...
        try (InputStreamReader reader = new InputStreamReader(resource.getInputStream()); CSVReader csvReader = new CSVReader(reader)) {
    ...
    public List<Company> readCompaniesFromCSV() {
        ...
        try (InputStream resourceInputStream = resource.getInputStream(); InputStreamReader reader = new InputStreamReader(resourceInputStream)) {
            CsvToBean<Company> csvToBean = new CsvToBeanBuilder<Company>(reader).withType(Company.class).withIgnoreLeadingWhiteSpace(true).build();
